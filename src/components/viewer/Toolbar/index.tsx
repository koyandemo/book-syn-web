...
const Toolbar = ({
  ...
  return (
    <>
      <div
        class={`w-full h-10 flex items-center justify-between border-b border-b-slate-300 shadow-sm text-xs select-none sticky top-0 bg-opacity-100 backdrop-blur z-10 bg-[white-smoke] pt-2`}
      >
        <div class='px-1 flex items-center space-x-1'>
          <button
            class={`enabled:hover:bg-slate-200 enabled:hover:text-black text-slate-500 disabled:text-slate-300 p-1 rounded-sm transition-all group relative focus:border-gray-400 focus:ring-0 focus:shadow outline-none border border-transparent`}
            onClick={() => setIsThumbsbarOpen(!isThumbsbarOpen())}
          >
            <VsMenu class='h-4 w-4' />
            {/* {isThumbsbarOpen() ? (
            ...
            >
              <input
                ...
                onKeyDown={(e:any) => {
                  switch (e.key) {
                    case "Enter":
                      store.pdfSlick?.gotoPage(Number(e.target.value))
                      break;
                    case "Down":
                    case "ArrowDown":
          ...
          <div class='flex items-center text-center space-x-2'>
            ...
            >
              <input
                ...
                type='text'
                value={wantedPageNumber()}
                class='block w-10 text-right rounded-sm border border-slate-300 focus:shadow focus:border-gray-400 focus:ring-0 outline-none text-xs pr-1 placeholder:text-gray-300 focus:placeholder:text-gray-400 placeholder:italic'
                onFocus={() => pageNumberRef!.select()}
                onChange={(e) => {
                  setWantedPageNumber(e.currentTarget.value)
                }}
